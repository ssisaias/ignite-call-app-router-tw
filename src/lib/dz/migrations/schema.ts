...
(table) => {
	return {
		provider_provider_account_id_key: uniqueIndex("accounts_provider_provider_account_id_key").on(table.provider, table.provider_account_id),
	}
});
...
(table) => {
	return {
		session_token_key: uniqueIndex("sessions_session_token_key").on(table.session_token),
	}
});
...
export const user_time_intervals = sqliteTable("user_time_intervals", {
	...
	user_id: text("user_id").notNull().references(() => users.id, { onDelete: "restrict", onUpdate: "cascade" } ),
});
